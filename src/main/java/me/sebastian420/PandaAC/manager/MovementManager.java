...
public class MovementManager {
    ...
    public static void read(ServerPlayerEntity player, MovementPacketData packet, long time) {
        ...
        boolean nearClimbable = PacketUtil.checkClimbable(world, packet);
        ...
        if (!player.isGliding()) {
            ...
            if (currentFluidState.getFluidState().isIn(FluidTags.WATER)) {
                ...
                if (!player.isSneaking() && !player.isCrawling()) {
                    ...
                    if (player.getHungerManager().getFoodLevel() > 6) {
                        ...
                        if (PacketUtil.checkPassage(world, packet)) {
            ...
            if (!playerData.getFlying()) {
                playerData.setElytraElevation(packet.getY() + playerData.getStoredSpeedVertical());
                playerData.setElytraMaxElevation(packet.getY() + playerData.getStoredSpeedVertical());
            }

            if (playerData.getElytraLastRocketTime() - time > 0) {
                if (packet.getY() < playerData.getElytraMaxElevation() && packet.getY() > playerData.getElytraElevation()) {
                    playerData.setElytraElevation(packet.getY());
                }
            }
        ...
        if( onGround || nearClimbable) {
            BlockState belowState = PacketUtil.checkBouncyBelow(world, packet);
            playerData.setLastAttached(packet.getX(), packet.getY(), packet.getZ(), belowState, player.getVelocity().getY(), time);
            ...
            playerData.setStoredSpeedVertical(playerData.getStoredSpeedVertical() * 0.75);
                ...
                packet.getY() > playerData.getLastY() &&
        ...
        if (playerMoveLength > ((speedPotential * SpeedLimits.SHORT_FUDGE + playerData.getStoredSpeed()) / 18)*speedMult) {
            ...
            playerData.setNew(packet, time);

    }


    public static void receiveTeleport(ServerPlayerEntity player, MovementPacketData teleportData) {
        PlayerMovementData playerData = getPlayer(player);
        playerData.teleport(teleportData.getX(), teleportData.getY(), teleportData.getZ(), System.currentTimeMillis());
    }

    public static void receiveVelocity(ServerPlayerEntity player, MovementPacketData velocityData) {
        PlayerMovementData playerData = getPlayer(player);
        double prevVelocity = playerData.getStoredSpeed();
        double prevVelocityVertical = playerData.getStoredSpeedVertical();
        playerData.setStoredSpeed(prevVelocity + (Math.abs(velocityData.getX()) + Math.abs(velocityData.getZ())) * 40);
        playerData.setStoredSpeedVertical(prevVelocityVertical + Math.abs(velocityData.getY()) * 40);
    }
}